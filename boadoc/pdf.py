...
from reportlab.platypus import (
    ...
    KeepTogether, CondPageBreak, Paragraph as _Paragraph, SimpleDocTemplate)
...
from boadoc.style import PARAGRAPH_KEY
...
class Writer(DocFile):
    """
    Pdf writer is just an abstraction layer to the report lab writer.  It is built in concert
    with the writing feature of word writer to minimum the duplication between the tool.  So user
    developer can arrange a document once without and the choose the appropriate format to write.
    ...
    def __init__(self, doc=None, file_path='', **kwargs):
        DocFile.__init__(self)

        if doc:
            # Update the content from the document into the pdf write.
            self.__dict__.update(doc.__dict__)

        self.file_path = file_path

        self.myDocument = SimpleDocTemplate(file_path)
    ...
    def close(self):
        """
        Close and save the pdf write.
        :return:
        """
        self.myDocument.build(self.Story, onFirstPage=self.myFirstPage, onLaterPages=self.myLaterPages)
    ...
    def write_paragraph(self, texts, **kwargs):
        ...
        :return:
        """
        if PARAGRAPH_KEY.get(kwargs.get('style')):
            style = PARAGRAPH_KEY[kwargs.get('style')][1]

            #TODO: Need to account for special style.
        else:
            style = self.styles["Normal"]

            self.Story.append(Paragraph(texts, style))
            self.Story.append(Spacer(1, 2*inch))

    def write_heading(self, text, level=1):
        """
        Return a heading paragraph for header.
        :param text:
    ...
    def write_picture(self, image_path_or_stream='', **kwargs):
        ...
        im = Image(image_path_or_stream, width, height)
        self.Story.append(im)

    def write_page_break(self):
        #TODO: Add page break to pdf.
        pass
...
class ReportingDocTemplate(BaseDocTemplate):
    def __init__(self, *args, **kwargs):
        BaseDocTemplate.__init__(self, kwargs.get('file_path'), **kwargs)
